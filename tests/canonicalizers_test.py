...
from modules.canonicalizers import TEIFileCanonicalizer, GoogleSTTCanonicalTranslator, \
...
if __name__ == '__main__':
    ...
    for tei_reference_file_path, google_transcription_files_path, aws_transcription_files_path in zip(
        ...
        TEIFileCanonicalizer(tei_reference_file_path, tei_file).canonicalize()
        print('------------------------------------------------------------------------------------------------------')
        print('Translating {} Google transcription file to canonical format...'.format(google_transcription_files_path))
        GoogleSTTCanonicalTranslator(google_transcription_files_path, tei_file).canonicalize()
        print('------------------------------------------------------------------------------------------------------')
        print('Translating {} AWS transcription file to canonical format...'.format(aws_transcription_files_path))
        AWSTranscribeCanonicalTranslator(aws_transcription_files_path, tei_file).canonicalize()
